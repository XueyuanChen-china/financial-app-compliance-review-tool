...
from compliance_review.compilation.validator import validate_applicability_expression
...
def test_source_registry_extracts_md_txt_pdf_docx_with_provenance(tmp_path: Path) -> None:
    ...
    registry = SourceRegistryBuilder().build(
        [tmp_path], {tmp_path.as_posix(): "country_regulator"}
    )
